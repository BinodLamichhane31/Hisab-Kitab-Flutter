...
class GyroscopeSensorTestWidget extends StatefulWidget {
  const GyroscopeSensorTestWidget({super.key});

  @override
  State<GyroscopeSensorTestWidget> createState() =>
      _GyroscopeSensorTestWidgetState();
}

class _GyroscopeSensorTestWidgetState extends State<GyroscopeSensorTestWidget> {
  final GyroscopeSensorService _gyroscopeSensorService =
      serviceLocator<GyroscopeSensorService>();
  final RotationService _rotationService = serviceLocator<RotationService>();
  ...
  Future<void> _toggleListening() async {
    if (_isListening) {
      ...
      _rotationService.stopListening();
      setState(() {
        _isListening = false;
      });
    } else {
      await _gyroscopeSensorService.startListening();
      await _rotationService.startListening(context);
      setState(() {
        _isListening = true;
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gyroscope Sensor Test'),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      'Rotate your device to trigger shop switching',
                      style: Theme.of(
                        context,
                    ...
                    ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            _isListening ? Colors.red : Colors.orange,
                        foregroundColor: Colors.white,
                      ),
                      child: Text(
